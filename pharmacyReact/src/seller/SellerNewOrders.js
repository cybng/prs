...
export default function AllProductList() {
   ...
   const [allApproved,setAllApproved] = useState(false);
    const getCsvData=async()=>{
        const sellerId = {requestId:auth.user._id}
        const res=await axios.post("http://localhost:3000/api/requestdata",sellerId);
        console.log(res);
        setAllData(res.data);
      }
    useEffect(()=>{
    ...
    const approvalBtn=async(approvalId)=>{
        ...
        const res = await axios.post("http://localhost:3000/api/approvalavaility",approval)
                    ...
                    <table className="min-w-full bg-white dark:bg-gray-800">
                        <thead>
                            <tr className="w-full h-16  bg-gray-200 border-gray-300 dark:border-gray-200 border-b py-8">
                                ...
                                <th className="text-gray-600 dark:text-gray-400 font-normal pr-6 text-left text-sm tracking-normal leading-4">SKU CODE</th>
                                <th className="text-gray-600 dark:text-gray-400 font-normal pr-6 text-left text-sm tracking-normal leading-4">Product Name</th>
                                <th className="text-gray-600 dark:text-gray-400 font-normal pr-6 text-left text-sm tracking-normal leading-4">Quantity</th>
                        ...
                        {allData.map((approval)=>(
                            <tr className="h-14 border-gray-300 dark:border-gray-200 border-b">
                                ...
                                <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 dark:text-gray-100 tracking-normal leading-4">{approval.skuCode}</td>
                                <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 dark:text-gray-100 tracking-normal leading-4">{approval.productName}</td>
                                <td className="pr-6 whitespace-no-wrap">
                                    <div className="flex items-center">
                                        
                                        <p className="ml-2 text-gray-800 dark:text-gray-100 tracking-normal leading-4 text-sm">{approval.quantity}</p>
                                    </div>
                                </td> 
                                ...
                                <td className="pr-8 relative">
                                    ...
                                    onClick={(e)=>approvalBtn(approval.product)}
                                    >
                                        Approved
                                    </button> 
                                </td>
